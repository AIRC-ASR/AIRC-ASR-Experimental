...
class Pyphones:
    ...
    def get_the_homophones_as_list(self):
        homophones = []
        homophone_pairs = self.get_the_homophones()[self.word]
        for pair in homophone_pairs:
            homophones.extend(pair)
        return homophones

'''
py = Pyphones("see")
homophones = py.get_the_homophones()
homophones_list = py.get_the_homophones_as_list()
print(homophones)
